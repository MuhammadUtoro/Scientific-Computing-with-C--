...
//      sin( w t )
SimTK::Real myFunction(const SomeStuff& stuff, SimTK::Real t) {
    const SimTK::Real w = stuff.w; // get the period
    return std::sin(w*t);
}
...
//     (1-cos(w t))/w
SimTK::Real analyticIntegral(const SomeStuff& stuff, SimTK::Real t) {
    const SimTK::Real w = stuff.w; // get the period
...
const SimTK::Real startTime = 0;
const SimTK::Real finalTime = 1;
const SimTK::Real reportInterval = 0.1;
int main () {
    try {
        SomeStuff myStuff; // constant data to pass to myFunction()
        myStuff.w = SimTK::Pi/2; 
        ...
        while (true) {
            const SimTK::Real prevT = integ.getTime();
            ...
            const SimTK::Real t = state.getTime();
            const SimTK::Real numerical = state.getZ()[0];
            const SimTK::Real analytic = analyticIntegral(myStuff, state.getTime());
...
class MySystemGuts : public SimTK::System::Guts {
    ...
    int realizeTopologyImpl(State& state) const override {
        ...
        const SimTK::Vector zInit(1, 0.); // initial value for the one z we want
